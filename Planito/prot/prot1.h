...
#define PROT_CONST 34567

static NSString * const kReceiptBundleVersion = @"1.20";
static NSString * const kReceiptBundleIdentifier = @"com.mifki.Planito";
...
static inline NSData* dsfsdf43rewf7sjdh() //mac address
{
...
static inline NSData* dsf2390hklq9023() //apple root cert
{
	OSStatus status;
...
static inline NSDictionary* jkhy87yguhuijou87yuh (NSString *receiptFilePath, int *asdasf) //dict for receipt
{
	NSData * rootCertData = dsf2390hklq9023();
	...
	const char * receiptPath = [receiptFilePath fileSystemRepresentation];
...
static inline BOOL oiuy8712wqadwq(int *asdasd)
{
    ...
    OSStatus status = SecStaticCodeCreateWithPath((CFURLRef)[NSURL fileURLWithPath:[[NSBundle mainBundle] bundlePath]], kSecCSDefaultFlags, &staticCode);
    ...
    NSDictionary *d = jkhy87yguhuijou87yuh([[[NSBundle mainBundle] appStoreReceiptURL] path], asdasd);
    ...
    NSData *guidData = dsfsdf43rewf7sjdh();
	if (guidData == nil) {
		exit(173);
		[1 release];
        return NO;
	...
	[input appendData:[d objectForKey:kReceiptOpaqueValueKey]];
	[input appendData:[d objectForKey:kReceiptBundleIdentiferDataKey]];
	
	NSMutableData *hash = [NSMutableData dataWithLength:SHA_DIGEST_LENGTH];
	SHA1([input bytes], [input length], [hash mutableBytes]);
	
	if (!([kReceiptBundleIdentifier isEqualToString:[d objectForKey:kReceiptBundleIdentiferKey]] &&
          [kReceiptBundleVersion isEqualToString:[d objectForKey:kReceiptVersionKey]] &&
          [hash isEqualToData:[d objectForKey:kReceiptHashKey]]) )
	{
		exit(173);
		[2 release];
        return NO;
...
#define CHECK_PROT oiuy8712wqadwq